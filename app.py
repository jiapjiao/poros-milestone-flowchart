...
for main in selected_main:
    ...
    for child in children:
        if child not in display_products:
            display_products.append(child)
...
for i, product in enumerate(display_products):
    ...
    # 水平时间线
    dates = []
    if pd.notna(row.get("起始日期")): dates.append(row["起始日期"])
    if pd.notna(row.get("中程日期")): dates.append(row["中程日期"])
    if pd.notna(row.get("结束日期")): dates.append(row["结束日期"])
